...
#include "ff9/src/diskio.h"			 /* Common include file for FatFs and disk I/O layer */
#include "ff9/src/option/syscall.c"  /* For multi-threading support */
...
{
  spiReadWrite(SD_SPI, 0, buff, bc, Speed, selectCSFatFS);
}
...
{
  spiReadWrite(SD_SPI, buff, 0, bc, Speed, selectCSFatFS);
}

// Select = 0/1
int selectCSFatFS(int select) 	/* 1:OK, 0:Timeout */
{
  if (select)
  {
	BYTE d;
